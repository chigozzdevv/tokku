...
async function checkRounds() {
  try {
    ...
    const roundsByStatus = await Round.aggregate([
    ...
    recentRounds.forEach((round) => {
      console.log(
        `  [${round.status}] ${round.marketId} #${round.roundNumber}`,
      );
    ...
    for (const market of markets) {
      const latest = await Round.findOne({ marketId: market._id })
        .sort({ roundNumber: -1 })
        .select("roundNumber status openedAt")
        .lean();
      console.log(
        `  ${market.name}: Round #${latest?.roundNumber || 0} (${latest?.status || "NONE"})`,
      );
    }
