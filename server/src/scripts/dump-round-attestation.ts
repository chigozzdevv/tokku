...
const TEE_PUBKEY_HEX =
  "045d46d0709c22ee95239e903ec5fe4929c321908ea4a35787d2a5cfda275a5116451f29b8cbfff2d53f46e8f0e6718c0da84e6956f6bef91fc9bc7f0a3ee9b405";
...
async function main() {
  ...
  const sigBytes =
    sigHex && /^[0-9a-fA-F]+$/.test(sigHex)
      ? Buffer.from(sigHex, "hex")
      : Buffer.alloc(0);
  const chBytes =
    chHex && /^[0-9a-fA-F]+$/.test(chHex)
      ? Buffer.from(chHex, "hex")
      : Buffer.alloc(0);
  ...
  console.log(
    "public_key_equals_TEE_PUBKEY?",
    pub.toLowerCase() === TEE_PUBKEY_HEX.toLowerCase(),
  );
  console.log(
