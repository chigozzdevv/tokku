...
async function main() {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const state =
      erState?.inputsHash && !/^0+$/.test(erState.inputsHash)
        ? erState
        : baseState;
