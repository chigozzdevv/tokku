...
async function main() {
  ...
  if (!name || !typeKey || !(typeKey in MT) || !mint || isNaN(index)) {
    throw new Error(
      "Usage: --name=... --type=PICK_RANGE|... --mint=<pubkey> --index=<u16> [--edge=0..10000] [--partition=2|4|10] [--skip=true]",
    );
  }
  const typeCode = MT[typeKey as MarketTypeKey];

  const admin = getAdminKeypair();
  const svc = new TokkuProgramService();
  const mintPk = new PublicKey(mint);

  const { signature, marketPda } = skipOnchain
    ? {
        signature: "skipped",
        marketPda: await svc.getMarketPdaByIndex(admin.publicKey, index),
      }
