...
export async function initializeJobs() {
  ...
  autoOpenInterval = setInterval(
    autoOpenRounds,
    config.ROUND_RELEASE_INTERVAL_SECONDS * 1000,
  );
  releaseInterval = setInterval(
    releaseQueuedRounds,
    config.ROUND_RELEASE_POLL_SECONDS * 1000,
  );
  ...
  logger.info(
    { intervalSeconds: config.ROUND_RELEASE_INTERVAL_SECONDS },
    "Auto-open scheduler started",
  );
  logger.info(
    { intervalSeconds: config.ROUND_RELEASE_POLL_SECONDS },
    "Release queued scheduler started",
  );
  logger.info("Cleanup old rounds scheduler started (interval: 1h)");
  logger.info(
    { intervalMs: recoveryIntervalMs },
    "Stalled round recovery scheduler started",
  );
