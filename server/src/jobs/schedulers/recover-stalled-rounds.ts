...
async function requeueLockJob(roundId: string) {
  ...
  if (existingJob) {
    ...
    try {
      ...
      logger.error(
        { roundId, jobId, error },
        "Failed to remove stale lock job",
      );
      return false;
    }
  }
...
export async function recoverStalledRounds() {
  try {
    const now = Date.now();
    const predictingGraceMs =
      (config.ROUND_DURATION_SECONDS + config.LOCK_DURATION_SECONDS + 60) *
      1000;
    const queuedGraceMs =
      (config.ROUND_RELEASE_INTERVAL_SECONDS +
        config.ROUND_QUEUE_BUFFER_SECONDS +
        60) *
      1000;
    ...
    const stalePredicting = await Round.find({
      ...
      .select("_id openedAt marketId roundNumber")
      .lean();
    ...
    const staleQueued = await Round.find({
      ...
      .select(
        "_id queuedAt marketId roundNumber releaseGroupId scheduledReleaseAt",
      )
      .lean();
