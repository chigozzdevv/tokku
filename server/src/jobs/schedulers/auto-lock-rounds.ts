...
export async function autoLockRounds() {
  try {
    ...
    const lockThreshold = new Date(
      now.getTime() - config.ROUND_DURATION_SECONDS * 1000,
    );

    const roundsToLock = await Round.find({
      status: RoundStatus.PREDICTING,
      openedAt: { $lte: lockThreshold },
    }).lean();
    ...
    for (const round of roundsToLock) {
      ...
      if (existingJob) {
        ...
        if (ACTIVE_JOB_STATES.has(state)) {
          logger.debug(
            { roundId, jobState: state },
            "Lock job already pending, skipping",
          );
          continue;
        }
        try {
          ...
          logger.debug(
            { roundId, jobState: state },
            "Removed stale lock job before rescheduling",
          );
        } catch (error) {
          logger.error(
            { roundId, jobState: state, error },
    ...
    if (roundsToLock.length > 0) {
      logger.info(
        { count: roundsToLock.length },
        "Auto-lock scheduler processed rounds",
      );
    }
  } catch (error) {
