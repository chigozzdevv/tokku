    ...
    if (this.running && this.runningTimestamp) {
      ...
      if (elapsed > this.TIMEOUT_MS) {
        logger.warn(
          ...
          "Auto-open scheduler lock timeout, forcing release",
        );
        this.release();
      } else {
        return false;
    ...
    for (const market of markets) {
      ...
      if (latest) {
        if (latest.status === RoundStatus.PREDICTING) {
          logger.debug(
            { marketId: market._id, roundId: latest._id },
            "Market already has active round, skipping queue",
          );
          continue;
        }
        if (latest.status === RoundStatus.LOCKED) {
          ...
          if (betCount === 0) {
            try {
              ...
              logger.info(
                { roundId: latest._id, marketId: market._id },
                "Expired stale locked round (no bets)",
              );
            } catch (e) {
              logger.error(
                { roundId: latest._id, err: e },
                "Failed to expire stale locked round",
              );
            }
          } else {
            logger.debug(
              { marketId: market._id, roundId: latest._id },
              "Locked round awaiting settlement; queuing next",
            );
          }
        }

        if (latest.status === RoundStatus.QUEUED) {
          const scheduled = latest.scheduledReleaseAt
            ? new Date(latest.scheduledReleaseAt).getTime()
            : null;
          if (
            scheduled &&
            Math.abs(scheduled - scheduledReleaseAt.getTime()) < intervalMs
          ) {
            logger.debug(
              { marketId: market._id, roundId: latest._id },
              "Round already queued for upcoming batch",
            );
            continue;
          }
        }
      ...
      try {
        const round = await roundsService.queueRound(
          String(market._id),
          scheduledReleaseAt,
          releaseGroupId,
        );
        if (round.status === RoundStatus.QUEUED) {
          ...
          logger.debug(
            { marketId: market._id, status: round.status },
            "Queue skipped due to existing round state",
          );
        }
      } catch (e) {
