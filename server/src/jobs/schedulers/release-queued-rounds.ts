...
export async function releaseQueuedRounds() {
  ...
  try {
    ...
    for (const round of dueRounds) {
      const key =
        (round as any).releaseGroupId || `manual-${(round as any)._id}`;
      if (!groups.has(key)) {
        groups.set(key, []);
      }
    ...
    for (const [groupId, rounds] of groups.entries()) {
      logger.info(
        { releaseGroupId: groupId, count: rounds.length },
        "Releasing queued round batch",
      );
      ...
      await Promise.allSettled(
        rounds.map(async (round) => {
          try {
            ...
            logger.info(
              { roundId: (round as any)._id, releaseGroupId: groupId },
              "Released queued round",
            );
          } catch (reason: any) {
            logger.error(
              {
      ...
      if (successfulRoundIds.length > 0) {
        const groupScheduled = rounds[0]?.scheduledReleaseAt
          ? new Date(rounds[0].scheduledReleaseAt as any)
          : new Date();
        await Round.updateMany(
          { _id: { $in: successfulRoundIds } },
          { $set: { openedAt: groupScheduled, releasedAt: groupScheduled } },
        );
        logger.info(
          { releaseGroupId: groupId, roundIds: successfulRoundIds },
          "Synchronized batch timestamps",
        );

        const lockAt =
          groupScheduled.getTime() + config.ROUND_DURATION_SECONDS * 1000;
