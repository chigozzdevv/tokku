...
async function processBetSettlementJob(job: Job<SettleBetsJobData>) {
  ...
  try {
    ...
    const outcome =
      typeof (round as any).outcome === "string"
        ? JSON.parse((round as any).outcome as any)
        : (round as any).outcome;
    ...
    if (isDelegated) {
      ...
      await tokkuProgram.ensureRoundUndelegated(
        marketPubkey,
        (round as any).roundNumber,
        adminKeypair,
      );
      logger.info(
        ...
        "Round undelegated before settlement",
      );
    }

    await tokkuProgram.waitForRoundOutcomeOnBase(
      marketPubkey,
      (round as any).roundNumber,
    );

    for (const bet of pendingBets as any[]) {
      const won = checkBetWon(
        bet.selection as any,
        outcome,
      ...
      if (won) {
        await updateLeaderboard(
          String(bet.userId._id ?? bet.userId),
          Number(bet.stake),
          payout,
        );
      } else {
      ...
      logger.info(
        { betId: bet._id, won, payout },
        "Bet settled (on-chain + DB)",
      );
    }

    await Round.updateOne(
      { _id: roundId },
      { $set: { status: RoundStatus.SETTLED } },
    );

    try {
      await tokkuProgram.settleRound(
        marketPubkey,
        round.roundNumber,
        adminKeypair,
      );
    ...
    const stillDelegated = Boolean(
      (refreshed as any)?.delegateTxHash &&
        !(refreshed as any)?.undelegateTxHash,
    );
    if (stillDelegated) {
      ...
      for (
        let attempt = 0;
        attempt < maxUndelegateAttempts && !undelegated;
        attempt++
      ) {
        try {
          if (attempt > 0)
          ...
          logger.info(
            { roundId, erTxHash, baseTxHash, attempt },
            "Round committed and undelegated on base layer",
          );
          undelegated = true;
        } catch (e) {
          logger.error(
            { roundId, err: e, attempt, maxUndelegateAttempts },
            `Commit and undelegate failed (attempt ${attempt + 1}/${maxUndelegateAttempts})`,
          );
          if (attempt === maxUndelegateAttempts - 1) {
            ...
            logger.error(
              { roundId },
              "CRITICAL: Round failed to undelegate after all retries",
            );
          }
        }
      }
    ...
    if (isRetryableSettlementError(errorMessage)) {
      logger.warn(
        { roundId, error: errorMessage },
        "Bet settlement not ready; will retry",
      );
      throw err;
    }
    if (!isOnchainTxError(errorMessage)) {
      logger.error(
        { roundId, error: errorMessage },
        "Bet settlement failed (non-chain); will retry",
      );
      throw err;
    }
    logger.error(
      { roundId, error: errorMessage },
      "Bet settlement failed, attempting refunds",
    );
    ...
    if (!round) {
      logger.error(
        { roundId },
        "Bet settlement failed and round not found for refund",
      );
      return;
    }
    ...
    if (!marketCfg.mintAddress) {
      logger.error(
        { roundId },
        "Bet settlement failed and mintAddress missing; skipping on-chain refunds",
      );
    }
    ...
    if (pending.length > 0) {
      const adminKeypair = getAdminKeypair();
      const marketPubkey = new PublicKey(marketCfg.solanaAddress);
      const mint = marketCfg.mintAddress
        ? new PublicKey(marketCfg.mintAddress)
        : null;
      ...
      try {
        ...
        logger.error(
          { roundId, error: msg },
          "Skipping on-chain refunds: round is still delegated",
        );
        throw e;
      }
      ...
      for (const bet of pending as any[]) {
        ...
        if (!wallet || !mint) {
          logger.error(
            ...
            "Skipping on-chain refund: missing wallet or mint",
          );
        } else {
          try {
            ...
            logger.info(
              { betId: bet._id, roundId },
              "On-chain refund completed after settlement failure",
            );
          } catch (refundErr: any) {
            const refundMsg =
              refundErr instanceof Error
                ...
                : String(refundErr);
            logger.error(
              { betId: bet._id, roundId, error: refundMsg },
              "On-chain refund failed after settlement failure",
            );
          }
        }
        ...
        if (refunded) {
          ...
          logger.info(
            { betId: bet._id, roundId },
            "Bet marked refunded in database after successful on-chain refund",
          );
        }
      }
      ...
      logger.error(
        { roundId },
        "Round marked FAILED after settlement failure and refund attempts",
      );
    }
  }
}
...
function checkBetWon(
  ...
  if (outcome.Numeric) {
    ...
    switch (selection.type) {
      ...
      case "parity":
        return (
          (value % 2 === 0 && selection.value === "even") ||
