...
export class BetsService {
  async createBetTransaction(
    ...
    this.validateSelection(
      selection,
      (round.marketId as any).type as MarketType,
    );

    const marketConfig = getMarketConfig(
      (round.marketId as any).config as unknown,
    );
    ...
    try {
      ...
      throw new ValidationError(
        "Invalid wallet address in session; please reconnect your wallet",
      );
    }
    ...
    const selectionEncoded = this.encodeSelection(
      selection,
      (round.marketId as any).type as MarketType,
    );
    ...
    logger.info(
      { userId, roundId, stake, betPda: betPda.toString() },
  ...
  async confirmBet(
    ...
    if (!round) throw new NotFoundError("Round");
    if (
      round.status !== RoundStatus.PREDICTING &&
    ...
    logger.error(
      { userId, roundId, txSignature, betPda },
      "Bet confirmation failed: on-chain bet account not found",
    );
    throw new ValidationError(
      "Bet transaction not confirmed on-chain; please try again",
    );
  }

  private encodeSelection(
    selection: any,
    marketType: MarketType,
  ): { kind: number; a: number; b: number; c: number } {
    switch (marketType) {
      ...
      case MarketType.SHAPE_COLOR:
        return {
          kind: 6,
          a: selection.shape ?? 255,
          b: selection.color ?? 255,
          c: selection.size ?? 255,
        };
  ...
  private calculateOdds(
    ...
    const fromEqualBins = (n: number) =>
      Math.max(1, Math.floor(n * edgeFactor * 100) / 100);
    ...
    switch (marketType) {
      case MarketType.PICK_RANGE:
        if (selection.type === "range") {
          ...
          if (width > 0 && 100 % width === 0) return fromEqualBins(100 / width);
          return fromProbability(width, 100);
        }
        if (selection.type === "single") return fromEqualBins(100);
      ...
      case MarketType.PATTERN_OF_DAY: {
        ...
        const idx =
          typeof selection.patternId === "number" ? selection.patternId : 6;
      ...
      case MarketType.COMMUNITY_SEED: {
        const t = Math.max(
          0,
          Math.min(8, selection.tolerance ?? selection.t ?? 0),
        );
  ...
  private decodeSelection(
    encoded: { kind: number; a: number; b: number; c: number },
    marketType: MarketType,
  ): any {
    switch (marketType) {
      case MarketType.PICK_RANGE:
        if (encoded.kind === 0)
          return { type: "range", min: encoded.a, max: encoded.b };
  ...
  private validateSelection(selection: any, marketType: MarketType) {
    ...
    switch (marketType) {
      ...
      case MarketType.PATTERN_OF_DAY: {
        ...
        if (!Number.isInteger(pid) || pid < 0 || pid > 6)
          throw new ValidationError("Invalid pattern selection");
        break;
      }
      case MarketType.SHAPE_COLOR: {
        const validByte = (n: any) =>
          n === undefined || (Number.isInteger(n) && n >= 0 && n <= 255);
        if (
          !validByte(selection.shape) ||
          !validByte(selection.color) ||
          !validByte(selection.size)
        ) {
    ...
    const [bets, total] = await Promise.all([
      Bet.find(where)
        .populate({
          path: "roundId",
          select:
            "id roundNumber status settledAt revealedAt revealTxHash marketId",
  ...
  async getBetStats(userId?: string, marketId?: string) {
    ...
    const [totalBets, wonBets, totalStakeAgg, totalPayoutAgg, pendingBets] =
      await Promise.all([
        ...
        Bet.aggregate([
          { $match: { ...match, status: BetStatus.WON } },
  ...
  async refundBets(roundId: string, reason: string) {
    ...
    const marketCfg = getMarketConfig(
      ...
      bets.map(async (bet: any) => {
        ...
        if (wallet) {
          try {
            ...
            refunded = true;
            logger.info(
              { betId: bet._id, roundId },
              "On-chain refund completed via admin refund endpoint",
            );
          } catch (err: any) {
            const msg = err instanceof Error ? err.message : String(err);
            logger.error(
              { betId: bet._id, roundId, error: msg },
              "On-chain refund failed via admin refund endpoint",
            );
          }
        } else {
          logger.error(
            { betId: bet._id, roundId },
            "Skipping on-chain refund via admin endpoint: missing wallet",
          );
        }
        ...
        if (refunded) {
          ...
          logger.info(
            { betId: bet._id, roundId, reason },
            "Bet refunded in database after successful on-chain refund",
          );
  ...
  private async processBetConfirmation(
    ...
    try {
      ...
      if (decodedBet?.selection) {
        ...
        finalSelection = this.decodeSelection(
          encoded,
          (round.marketId as any).type,
        );
      }
    } catch (err) {
    ...
    const houseEdgeBps: number =
      typeof cfg?.houseEdgeBps === "number" ? cfg.houseEdgeBps : 0;
    ...
    logger.info(
      {
        betId: (bet as any).id || (bet as any)._id.toString(),
        userId,
        roundId,
        txSignature,
        betPda,
