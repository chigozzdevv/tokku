...
export class BetsController {
  ...
  confirmBet = asyncHandler(
    async (request: FastifyRequest, reply: FastifyReply) => {
      if (!request.user) {
        throw new Error("User not authenticated");
      }

      const { roundId, selection, stake, txSignature, betPda } =
        request.body as {
          roundId: string;
          selection: any;
          stake: number;
      ...
      logger.info(
        { userId: request.user.id, roundId, txSignature, betPda },
        "Confirm bet request",
      );
