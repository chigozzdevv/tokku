...
export class AnalyticsService {
  async getOverview() {
    ...
    const roundsForTiming = await Round.find({
      lockedAt: { $ne: null },
      revealedAt: { $ne: null },
      settledAt: { $ne: null },
    })
    ...
    const verificationRate =
      attestationsTotal > 0
        ? (attestationsVerified / attestationsTotal) * 100
        : 0;
  ...
  async getMarketMetrics() {
    ...
    const perMarketRounds = await Round.aggregate([
      { $group: { _id: "$marketId", count: { $sum: 1 } } },
    ]);
    const roundsSettledByMarket = await Round.aggregate([
      { $match: { status: "SETTLED" } },
      { $group: { _id: "$marketId", count: { $sum: 1 } } },
    ]);

    const roundsByMarketMap = new Map(
      perMarketRounds.map((r: any) => [String(r._id), r.count]),
    );
    const settledByMarketMap = new Map(
      roundsSettledByMarket.map((r: any) => [String(r._id), r.count]),
    );
    ...
    const result = markets.map((m: any) => {
      ...
      const avgBetsPerRound =
        Number(totalRounds) > 0
          ? Math.round((Number(totalBets) / Number(totalRounds)) * 100) / 100
          : 0;
  ...
  async getTimeSeries(days: number = 14, granularity: Granularity = "daily") {
    ...
    const bets = await Bet.find({ createdAt: { $gte: since } })
      .select("createdAt stake payout")
      .sort({ createdAt: 1 })
      .lean();

    const byKey = new Map<
      string,
      { date: string; bets: number; volume: number; payout: number }
    >();
    const toKey = (d: Date) => {
      if (granularity === "weekly") {
        ...
        const weekNo = Math.ceil(
          ((wd.getTime() - yearStart.getTime()) / 86400000 + 1) / 7,
        );
  ...
  private async countActiveUsersSince(since: Date) {
    const distinct = await Bet.distinct("userId", {
      createdAt: { $gte: since },
    });
  ...
  async getMarketHealth() {
    ...
    const marketHealth = await Promise.all(
      markets.map(async (market: any) => {
        ...
        const [activeRounds, totalRounds, settledRounds, recentBets] =
          await Promise.all([
        ...
        const volumeGrowth =
          avgVolume7dPerDay > 0
            ? ((volume24h - avgVolume7dPerDay) / avgVolume7dPerDay) * 100
            : 0;

        const settlementRate =
          totalRounds > 0 ? (settledRounds / totalRounds) * 100 : 0;
    ...
    const marketStats = await Promise.all(
      markets.map(async (market: any) => {
        ...
        const [bets24h, bets48h, activeRounds] = await Promise.all([
          Bet.find({ marketId: market._id, createdAt: { $gte: last24h } })
            .select("stake")
            .lean(),
          Bet.find({
            marketId: market._id,
        ...
        const volumePrevious24h = bets48h.reduce(
          (sum, bet) => sum + Number(bet.stake),
          0,
        );
  ...
  async getRoundPerformanceMetrics() {
    ...
    rounds.forEach((r: any) => {
      if (r.openedAt && r.lockedAt && r.revealedAt && r.settledAt) {
        totalDuration +=
          new Date(r.settledAt).getTime() - new Date(r.openedAt).getTime();
        totalLockToReveal +=
          new Date(r.revealedAt).getTime() - new Date(r.lockedAt).getTime();
        totalRevealToSettle +=
          new Date(r.settledAt).getTime() - new Date(r.revealedAt).getTime();
        validRounds++;
      }
    });
    ...
    return {
      avgDuration:
        validRounds > 0 ? Math.round(totalDuration / validRounds) : 0,
      avgLockToReveal:
        validRounds > 0 ? Math.round(totalLockToReveal / validRounds) : 0,
      avgRevealToSettle:
        validRounds > 0 ? Math.round(totalRevealToSettle / validRounds) : 0,
