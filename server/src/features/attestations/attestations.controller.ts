...
export class AttestationsController {
  getAttestationByHash = asyncHandler(
  ...
  getAttestationsByRound = asyncHandler(
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { roundId } = request.params as any;

      const attestations =
        await attestationsService.getAttestationsByRound(roundId);

      return success(reply, attestations);
    },
  );

  verifyAttestation = asyncHandler(
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { hash } = request.params as any;
  ...
  getAttestationProof = asyncHandler(
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { hash } = request.params as any;
      ...
      const proof = {
        ...
        solanaExplorer: attestation.txHash
          ? `https://explorer.solana.com/tx/${attestation.txHash}?cluster=devnet`
          : null,
  ...
  getAttestationStats = asyncHandler(
    async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  verifyteeIntegrity = asyncHandler(
