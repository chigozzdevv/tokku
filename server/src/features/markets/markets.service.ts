...
export class MarketsService {
    ...
    return markets.map(
      (m) =>
        ({
          ...
          _count: {
            rounds: rMap.get(String(m._id)) || 0,
            bets: bMap.get(String(m._id)) || 0,
          },
        }) as any,
    );
  }
  ...
  async getActiveRounds() {
    const rounds = await Round.find({ status: "PREDICTING" })
      .sort({ openedAt: 1 })
      .lean();
  ...
  async getMarketHistory(marketId: string, limit: number = 50) {
    ...
    const rounds = await Round.find({ marketId, status: "SETTLED" })
      .sort({ settledAt: -1 })
      .limit(limit)
      .lean();
    const betCounts = await Bet.aggregate([
  ...
  async updateHouseEdgeBps(
    ...
    if (
      ...
      adminKeypair.publicKey.toString().toLowerCase()
    ) {
    ...
    const signature = await tokkuProgram.setHouseEdgeBps(
      marketPubkey,
      houseEdgeBps,
      adminKeypair,
    );
  ...
  async updateHouseEdgeBpsByType(
    type: string,
    houseEdgeBps: number,
    user: { id: string; walletAddress: string },
  ) {
    ...
    for (const market of markets) {
      const cfg = getMarketConfig(market.config as unknown);
      const marketPubkey = new PublicKey(cfg.solanaAddress);
      const sig = await tokkuProgram.setHouseEdgeBps(
        marketPubkey,
        houseEdgeBps,
        adminKeypair,
      );
