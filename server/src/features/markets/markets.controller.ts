...
export class MarketsController {
  ...
  updateHouseEdge = asyncHandler(
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { marketId } = request.params as { marketId: string };
      const { houseEdgeBps } = request.body as { houseEdgeBps: number };
      const result = await marketsService.updateHouseEdgeBps(
        marketId,
        houseEdgeBps,
        request.user!,
      );
  ...
  updateHouseEdgeByType = asyncHandler(
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { type, houseEdgeBps } = request.body as {
        type: string;
        houseEdgeBps: number;
      };
      const result = await marketsService.updateHouseEdgeBpsByType(
        type,
        houseEdgeBps,
        request.user!,
      );
