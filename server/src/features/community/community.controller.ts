...
export class CommunityController {
  joinCommunityRound = asyncHandler(
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      return success(
        reply,
        participation,
        "Joined community round successfully",
      );
    },
  );

  getCommunityRoundParticipants = asyncHandler(
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { roundId } = request.params as any;

      const participants =
        await communityService.getCommunityRoundParticipants(roundId);

      return success(reply, participants);
    },
  );

  getUserCommunityHistory = asyncHandler(
    async (request: FastifyRequest, reply: FastifyReply) => {
      if (!request.user) {
        throw new Error("User not authenticated");
      }
      ...
      const history = await communityService.getUserCommunityHistory(
        request.user.id,
        options,
      );
  ...
  getCommunityStats = asyncHandler(
    async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  getCommunityLeaderboard = asyncHandler(
    async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  finalizeCommunityRound = asyncHandler(
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { roundId } = request.params as any;
  ...
  getRealTimeParticipants = asyncHandler(
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { roundId } = request.params as any;

      const participants =
        await communityService.getCachedCommunityRound(roundId);
