...
export class CommunityService {
  async joinCommunityRound(userId: string, roundId: string, byte: number) {
    ...
    const existingSeed = await CommunitySeed.findOne({
      userId,
      roundId,
    }).lean();
    ...
    logger.info(
      `User joined community round: ${userId} - Round: ${roundId} - Byte: ${byte}`,
    );
  ...
  async finalizeCommunityRound(roundId: string) {
    ...
    const seededWithDistance = await Promise.all(
      seeds.map(async (seed: any) => {
        ...
        await CommunitySeed.updateOne(
          { _id: seed._id },
          { $set: { distance, won: distance === 0 } },
        );
    ...
    const winners = seededWithDistance.filter(
      (seed: { won?: boolean }) => seed.won,
    );

    const closestWinners =
      winners.length > 0
        ? winners
        : seededWithDistance
  ...
  async getCommunityRoundParticipants(roundId: string) {
    ...
    const avgByte =
      seeds.reduce((sum: number, s: { byte: number }) => sum + s.byte, 0) /
      total;
  ...
  async getCommunityStats(userId?: string) {
    ...
    const [totalParticipations, totalWins, totalRounds, avg] =
      await Promise.all([
        ...
        CommunitySeed.aggregate([
          { $match: { ...userFilter, distance: { $ne: null } } },
    ...
    return {
      ...
      avgDistance: avg?.[0]?.avg
        ? Math.round((avg?.[0]?.avg || 0) * 100) / 100
        : 0,
    };
  }
  ...
  async getCommunityLeaderboard(limit: number = 50) {
    ...
    for (let i = 0; i < winStats.length; i++) {
      ...
      const user = await User.findById(stat._id)
        ...
        .lean();
      const totalParticipations = await CommunitySeed.countDocuments({
        userId: stat._id,
      });
      const winRate =
        totalParticipations > 0
          ? (stat.totalWins / totalParticipations) * 100
          : 0;
      leaderboard.push({
        rank: i + 1,
        user: user!,
