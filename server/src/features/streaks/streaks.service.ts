...
export class StreaksService {
  async createStreak(userId: string, marketId?: string, target?: number) {
    ...
    logger.info(
      `Streak created: ${streak.id} - User: ${userId} - Target: ${streakTarget}`,
    );
    ...
    for (const streak of activeStreaks) {
      if (roundResult === "won") {
        ...
        if (newStreakCount >= streak.target) {
          ...
          const completedStreak = await Streak.findById(
            (streak as any)._id,
          ).lean();
          ...
          logger.info(
            `Streak completed: ${streak.id} - User: ${userId} - Final count: ${newStreakCount}`,
          );
        } else {
          // Continue streak
          await Streak.updateOne(
            { _id: (streak as any)._id },
            { $set: { currentStreak: newStreakCount, lastRoundId: roundId } },
          );
          const updatedStreak = await Streak.findById(
            (streak as any)._id,
          ).lean();
        ...
        await Streak.updateOne(
          { _id: (streak as any)._id },
          { $set: { status: StreakStatus.FAILED, lastRoundId: roundId } },
        );
        ...
        logger.info(
          `Streak failed: ${streak.id} - User: ${userId} - Final count: ${streak.currentStreak}`,
        );
      }
  ...
  async getStreakLeaderboard(limit: number = 50) {
    ...
    return topStreaks.map((streak: any, index: number) => ({
      ...
      completionTime: streak.completedAt
        ? new Date(streak.completedAt).getTime() -
          new Date(streak.startedAt).getTime()
        : 0,
    }));
  }
  ...
  async validateStreakBet(
    ...
    if (target < 2 || target > config.MAX_STREAK_TARGET) {
      throw new ValidationError(
        `Streak target must be between 2 and ${config.MAX_STREAK_TARGET}`,
      );
    }
  ...
  async getStreakStats(userId?: string) {
    ...
    const [
      ...
      Streak.findOne({ ...where, status: StreakStatus.COMPLETED })
        .sort({ target: -1 })
        .lean(),
    ]);

    const completionRate =
      totalStreaks > 0 ? (completedStreaks / totalStreaks) * 100 : 0;
  ...
  async cleanupInactiveStreaks() {
    ...
    const res = await Streak.updateMany(
      { status: StreakStatus.ACTIVE, startedAt: { $lt: cutoffTime } },
      { $set: { status: StreakStatus.FAILED } },
    );
    const inactiveStreaks = {
      count: (res as any).modifiedCount ?? (res as any).matchedCount,
    } as any;
