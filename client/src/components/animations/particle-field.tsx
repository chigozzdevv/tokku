  ...
  useEffect(() => {
    ...
    const onResize = () => {
      ...
      fpsIntervalRef.current =
        prefersReducedRef.current || isMobileRef.current
          ? 1000 / 30
          : 1000 / 45;
    ...
    const tick = (ts?: number) => {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        const r = Math.max(
          0.4,
          p.size * pulse * (isMobileRef.current ? 0.85 : 1),
        );
    ...
    const onVisibility = () => {
      ...
      if (!hiddenRef.current && !rafRef.current)
        rafRef.current = requestAnimationFrame(tick);
      if (hiddenRef.current && rafRef.current) {
    ...
    const onIntersect: IntersectionObserverCallback = ([entry]) => {
      ...
      if (!pausedRef.current && !rafRef.current && !hiddenRef.current)
        rafRef.current = requestAnimationFrame(tick);
