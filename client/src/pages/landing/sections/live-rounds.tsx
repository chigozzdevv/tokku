  ...
  switch (round.market.type) {
    case "PICK_RANGE": {
      const partitions = Math.max(
        1,
        Math.min(20, Math.floor(asNumber(config.partitionCount, 2))),
      );
      ...
      return [
        {
          id: "color",
          label: "Match Color",
          odds: fromProbability(12, 72),
          coverage: "12/72",
        },
        {
          id: "shape-color",
          label: "Match Shape & Color",
          odds: fromProbability(3, 72),
          coverage: "3/72",
        },
        {
          id: "full",
          label: "Exact Shape + Color + Size",
          odds: fromProbability(1, 72),
          coverage: "1/72",
        },
      ];
      ...
      const options: RoundOption[] = [
        {
          id: "tol-2",
          label: "Distance ≤ 2",
          odds: fromProbability(toleranceCounts(2), 256),
          coverage: `${toleranceCounts(2)}/256`,
        },
        {
          id: "tol-1",
          label: "Distance ≤ 1",
          odds: fromProbability(toleranceCounts(1), 256),
          coverage: `${toleranceCounts(1)}/256`,
        },
        {
          id: "tol-0",
          label: "Exact Byte",
          odds: fromProbability(1, 256),
          coverage: "1/256",
        },
      ];
...
export function LiveRoundsSection() {
  ...
  const activeRounds = useMemo(() => {
    return rounds
      ...
      .sort(
        (a, b) =>
          (a.endsAt ?? Number.MAX_SAFE_INTEGER) -
    ...
    return (
      <Section id="live-rounds">
        <div className="container">
          <SectionHeader
            title="Live Rounds"
            sub="Upcoming and active betting rounds"
          />
          <div
            style={{
              padding: "4rem 0",
        ...
        {displayEntries.length > 0 && (
          ...
          >
            {displayEntries.map((entry, idx) => {
              ...
              const description =
                MARKET_DESCRIPTIONS[round.market.type] || "Place your bet";
              const options = buildRoundOptions(round).slice(0, 3);
              const tagText = "Live";
              const tagBackground =
                "color-mix(in oklab, var(--accent) 32%, transparent)";
              const openedTime =
                round.scheduledReleaseAt || round.openedAt
                  ? new Date(
                      round.scheduledReleaseAt || (round.openedAt as string),
                    ).toLocaleTimeString([], {
                      hour: "2-digit",
                      minute: "2-digit",
                    })
