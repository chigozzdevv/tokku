...
const ACCENT_POOL = [
  ...
  {
    bg: "rgba(248,196,113,0.08)",
    border: "rgba(248,196,113,0.25)",
    chart: "#fbbf24",
  },
  {
...
function createFilters(markets: MarketCard[]): MarketFilter[] {
  ...
  if (markets.some((market) => market.isActive)) {
    filters.push({
      id: "active",
      label: "Active",
      predicate: (market) => market.isActive,
    });
  }
  const maxBets = markets.reduce(
    (max, market) => Math.max(max, market._count?.bets ?? 0),
    0,
  );
  if (maxBets > 0) {
    filters.push({
      ...
      label: "Popular",
      predicate: (market) =>
        (market._count?.bets ?? 0) >= Math.max(3, Math.floor(maxBets * 0.6)),
    });
  }
...
function decorateMarkets(markets: MarketSummary[]): MarketCard[] {
  return markets.map((market, index) => {
    ...
    return {
      ...
      activityScore,
      description:
        DESCRIPTIONS[market.type] ?? "Provably fair attested market.",
    };
  });
}
...
export function MarketsPage() {
  ...
  const filteredMarkets = useMemo(() => {
    const filter =
      filters.find((item) => item.id === activeFilter) ?? filters[0];
      ...
      <div className="dashboard-panel-header">
        <div>
          ...
          <p className="dashboard-subtitle">
            Select a market to explore rounds, odds and analytics
          </p>
        </div>
      </div>
        ...
        {filteredMarkets.map((market) => (
          ...
          >
            <div className="dashboard-market-card-header">
              <span className="dashboard-market-chip">
                {humanize(market.type)}
              </span>
              <span
                className={`dashboard-market-status ${market.isActive ? "active" : "paused"}`}
              >
            ...
            <div className="dashboard-market-metrics">
              <div>
                ...
                <strong style={{ color: market.accent.chart, fontWeight: 900 }}>
                  {market._count?.rounds ?? 0}
                </strong>
              </div>
              <div>
              ...
              <div>
                ...
                >
                  <TrendChart
                    value={market.activityScore / 10}
                    color={market.accent.chart}
                    height={28}
                  />
                </div>
              </div>
