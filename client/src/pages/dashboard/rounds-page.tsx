...
const ACCENTS = [
  {
    bg: "rgba(185,246,201,0.08)",
    border: "rgba(185,246,201,0.25)",
    chart: "#62df98",
  },
  {
    bg: "rgba(138,180,248,0.08)",
    border: "rgba(138,180,248,0.25)",
    chart: "#8b8cf8",
  },
  {
...
function buildSelectionEntries(round: Round): RoundSelectionEntry[] {
  ...
  return options.map((option, idx) => ({
    ...
    timeRatio,
    trend: clamp(
      bets / 10 + 1 / Math.max(option.odds, 1) + (1 - timeRatio),
      0,
      3,
    ),
    bets,
...
function createFilters(entries: RoundSelectionEntry[]): RoundFilter[] {
  ...
  if (maxBets > 0) {
    filters.push({
      ...
      label: "Popular",
      predicate: (entry) =>
        entry.bets >= Math.max(2, Math.floor(maxBets * 0.6)),
    });
  }
  ...
  const highOddsThreshold =
    entries.reduce((sum, entry) => sum + entry.odds, 0) /
  ...
  const types = Array.from(
    new Set(entries.map((entry) => entry.round.market.type)),
  );
  types.forEach((type) => {
    filters.push({
      id: `type:${type}`,
  ...
  if (error) {
    return (
      <div className="dashboard-panel">
        <p>{error}</p>
        <button className="btn" onClick={fetchRounds}>
          Retry
        </button>
      </div>
    );
    ...
    <div className="dashboard-panel dashboard-panel-split">
      ...
      {startingSoonRounds.length > 0 && (
        <div className="starting-soon-section">
          <div className="starting-soon-header">
            ...
            <span className="starting-soon-subtitle">
              {startingSoonRounds.length}{" "}
              {startingSoonRounds.length === 1 ? "round" : "rounds"} about to
              begin
            </span>
          </div>
              ...
              return (
                <div key={round.id} className="starting-soon-card">
                  <div className="starting-soon-card-header">
                    <span className="dashboard-round-market-type">
                      {humanizeMarketType(round.market.type)}
                    </span>
                    <div className="starting-soon-pulse"></div>
                  </div>
                  <h3 className="starting-soon-card-title">
                    {round.market.name}
                  </h3>
                  <p className="dashboard-round-meta">
      ...
      {filteredEntries.length === 0 ? (
        <div className="dashboard-empty">
          No bets match this filter right now.
        </div>
      ) : (
        <div className="dashboard-round-grid">
          {filteredEntries.map((entry) => (
            <button
              ...
              onClick={() =>
                navigate(`/app/rounds/${entry.round.id}`, {
                  state: { selection: entry.selection, highlight: entry.id },
                })
              }
            >
              <div className="dashboard-round-card-header">
                <span className="dashboard-round-market-type">
                  {humanizeMarketType(entry.round.market.type)}
                </span>
                <div style={{ width: "100px", height: "28px" }}>
                  <TrendChart
                    value={entry.trend}
                    color={entry.accent.chart}
                    height={28}
                  />
                </div>
              </div>
              ...
              <div className="dashboard-round-stats">
                <div>
                  ...
                  <strong
                    className="dashboard-round-odds-bold"
                    style={{ color: entry.accent.chart }}
                  >
                    {entry.odds.toFixed(entry.odds >= 10 ? 1 : 2)}x
                  </strong>
                  {entry.coverage ? (
                    <span className="dashboard-round-coverage">
