...
export function MarketDetailPage() {
  ...
  const [roundFilter, setRoundFilter] = useState<"all" | "active" | "history">(
    "active",
  );
  ...
  const rounds = useMemo(() => {
    ...
    switch (roundFilter) {
      case "active":
        return list.filter(
          (round) => round.status === "PREDICTING" || round.status === "LOCKED",
        );
      case "history":
        return list.filter(
          (round) => round.status === "SETTLED" || round.status === "FAILED",
        );
      default:
        return list;
    }
  ...
  return (
    <div className="dashboard-stack">
      <section
        className="dashboard-panel dashboard-market-hero"
        style={{
          background:
            "linear-gradient(135deg, rgba(185,246,201,0.18), rgba(17,23,19,0.8))",
        }}
      >
        <div className="dashboard-market-hero-content">
          <div>
            <span className="dashboard-market-chip large">
              {market.type.replace(/_/g, " ")}
            </span>
            <h1 className="dashboard-title">{market.name}</h1>
            <p className="dashboard-subtitle">
              {typeof market.config === "string"
                ? "Configurable attested market"
                : "Live Solana market powered by Tokku engine."}
            </p>
          </div>
      ...
      <section className="dashboard-panel">
        <div className="dashboard-panel-header">
          <div>
            ...
            <p className="dashboard-subtitle">
              Browse recent rounds and jump into live betting
            </p>
          </div>
        ...
        {rounds.length === 0 ? (
          <div className="dashboard-empty">
            No rounds available for this view.
          </div>
        ) : (
                ...
                {rounds.map((round) => (
                  <tr key={round.id}>
                    ...
                    <td>
                      <Button
                        variant="primary"
                        onClick={() => navigate(`/app/rounds/${round.id}`)}
                      >
                        View round
                      </Button>
                    </td>
